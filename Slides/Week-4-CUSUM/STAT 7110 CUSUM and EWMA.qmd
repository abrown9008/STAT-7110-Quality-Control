...
$$ C^{+} = \max[0, \bar{x}_i - (\mu_0 + K) + C_{i-1}^{+}] $$ $$ C^{-} = \max[0, (\mu_0 - K) - \bar{x}_i + C_{i-1}^{-}] $$
...
$$ E[z_i] = E\bigg[\lambda\sum_{j=0}^{i-1}(1-\lambda)^{j}x_{i-j} + (1-\lambda)^iz_0\bigg] $$ $$ \implies \lambda\sum_{j=0}^{i-1}(1-\lambda)^{j}E[x_{i-j}] + (1-\lambda)^iE[z_0] $$
...
$$ UCL = \mu_0 + L\sqrt{\sigma^2_{x_i}\bigg(\frac{\lambda}{2-\lambda}\bigg)(1-(1-\lambda)^{2i})} $$ $$ LCL = \mu_0 - L\sqrt{\sigma^2_{x_i}\bigg(\frac{\lambda}{2-\lambda}\bigg)(1-(1-\lambda)^{2i})} $$

## The EWMA Chart

-   Notice as $i\rightarrow\infty$:

$$ UCL = \mu_0 + L\sqrt{\sigma^2_{x_i}\bigg(\frac{\lambda}{2-\lambda}\bigg)} $$ $$ LCL = \mu_0 - L\sqrt{\sigma^2_{x_i}\bigg(\frac{\lambda}{2-\lambda}\bigg)} $$
